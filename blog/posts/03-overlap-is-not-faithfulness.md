...
Compress-then-embed remains a hypothesis worth pursuing, but only behind a **multi-metric gate** I can re-run on the full 22-item corpus (12 arXiv + 10 YouTube). The arXiv slice proved the scorecard mechanics; the YouTube transcripts are the product-shaped stress test — long, messy, and full of names and numbers from talks I might actually ingest. Overlap metrics are a compass, not a contract. For a plain-language map of all four checks on the same rows, see [Did the Summary Keep the Important Bits?](./faithfulness-metrics-map.md).
